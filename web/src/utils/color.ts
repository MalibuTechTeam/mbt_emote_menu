...
/**
 * Anything a person might reasonably type or paste, as '#RRGGBB'.
 *
 * Accepts '#abc', '#aabbcc', a bare 'aabbcc', and rgb()/rgba() with either
 * commas or spaces. Alpha parses but is DROPPED -- callers here use the colour
 * as an opaque fill and apply their own alpha on top of it, so a colour
 * carrying its own would be applied twice. Returning hex is what tells the
 * person it went away.
 *
 * @returns '#RRGGBB' uppercase, or null when it is not a colour.
 */

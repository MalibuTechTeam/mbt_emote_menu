...
import {
  hexToHsv, hexToRgb, hsvToHex, hsvToRgb, isHexColor, parseColor, type HSV,
} from '../utils/color'
...
export function ColorPicker({ value, onChange, presets = [], footer, 'aria-label': label }: ColorPickerProps) {
  ...
  // The typed value, kept apart from the colour: half-finished input is not a
  // colour yet, and rewriting the field on every keystroke fights the typist.
    ...
    <div className={`mbt-cp${open ? ' is-open' : ''}`} ref={rootRef}>
      ...
      {open && createPortal(
        <div ref={popRef} className={`mbt-cp__pop${pos?.flip ? ' is-flipped' : ''}`}
          ...
          {/* Typed entry. Dragging is for finding a colour; this is for
              landing on one somebody handed you. */}
          <div className="mbt-cp__fields">
            <input
              className={`mbt-cp__hex${bad ? ' is-bad' : ''}`}
              value={typed ?? current}
              spellCheck={false}
              aria-label="Hex or rgb() value"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') commitTyped(typed ?? current)
            ...
            />
            {/* The same colour in the other notation people are handed it in,
                read-only: editing it would be a second source of truth. */}
            <span className="mbt-cp__rgb">
              {(() => {
